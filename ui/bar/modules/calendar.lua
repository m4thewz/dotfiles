...
local english_months = { "January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December" }

local function english_date(time)
  local date = os.date("*t", time or nil)
  local month = english_months[date.month]
  return helpers.colorizeText(month .. " " .. date.year, beautiful.blue)
end
...
local function date_widget(date, is_current, is_another_month)
  ...
  if is_current == true then
    text_color = beautiful.blue
  elseif is_another_month == true then
    text_color = beautiful.bg_light
  end
  ...
  return wibox.widget({
    ...
    forced_height = dpi(30),
    shape = gshape.circle,
    bg = is_current and beautiful.bg .. "75",
    widgets.text({
      halign = "center",
      size = 10,
      color = text_color,
...
function calendar:set_date(date)
  ...
  self.month.markup = english_date(time)
...
return function(s)
  s.calendar = awful.popup {
    ...
    placement = function(w)
      return awful.placement.bottom_right(w, {
        margins = {
          bottom = beautiful.wibar_height + 5,
          right = 5,
        }
      })
    end,
  ...
  awesome.connect_signal("calendar::minimize", function()
